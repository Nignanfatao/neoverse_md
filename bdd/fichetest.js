...
async function createFicheTestTable() {
  const client = await pool.connect();

  try {
    // Créez la table east si elle n'existe pas déjà
    await client.query(`
      CREATE TABLE IF NOT EXISTS fichetest(
    ...
    `);
    console.log('Table fichetest créée avec succès');
  } catch (error) {
    console.error('Erreur lors de la création de la table fichetest:', error);
  } finally {
    client.release();
  }
}
...
async function insertData() {
  ...
  try {
    for (let i = 1; i <= 12; i++) {
      const query = `
        INSERT INTO fichetest(e1, e2, e3, e4, e5, e6, e7, e8, e9, e10, e11, e12, e13, e14, e15, e16, e17)
...
async function getData(ide) {
  ...
  try {
   const query = 'SELECT e1, e2, e3, e4, e5, e6, e7, e8, e9, e10, e11, e12, e13, e14, e15, e16, e17 FROM fichetest WHERE id = $1';
